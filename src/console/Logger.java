...
/**
 * Cette classe est absolette et n'est pas fini.
 * Elle permet juste de voir les données du personnels et des patients
 * dans la console sous forme de tableau
 * 
 * @author Andy
 */
public abstract class Logger {
	...
	public static <E extends IPersonne> void loggerListObject(ArrayList<E> objects, String title) {
		...
		for (E object : objects) {
			...
			if (lengthLastName >= 3) {
				for (int i = 0; i < lengthLastName - object.getLastName().length(); i++) {
					System.out.print(" ");
				}
			} else {
				for (int i = 0; i < 3 - lengthLastName; i++) {
					System.out.print(" ");
				}
			}
			System.out.print(" |\n");
